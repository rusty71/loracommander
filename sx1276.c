 ...
 */
void SX1276OnTimeoutIrq(const struct timer_task *const timer_task);
 ...
 */
static volatile SX1276_t SX1276;
...
static struct timer_task HWTimers[3];
static bool timer_running[3];

static struct timer_task ledblink;

void TxTimeoutTimer_cb(const struct timer_task *const timer_task) {
    //~ UART_println("TX:fired");
    timer_running[0] = false;
    SX1276OnTimeoutIrq(timer_task);
}
void RxTimeoutTimer_cb(const struct timer_task *const timer_task) {
    //~ UART_println("RX:fired");
    timer_running[1] = false;
    SX1276OnTimeoutIrq(timer_task);
}
void RxTimeoutSyncWord_cb(const struct timer_task *const timer_task) {
    //~ UART_println("SYNC:fired");
    timer_running[2] = false;
    SX1276OnTimeoutIrq(timer_task);
}

void TimerStart(TimerType_t timer) {
    //~ UART_print("start:");
    switch(timer) {
        case RxTimeoutTimer:
            //~ UART_print("RX:");
    ...
    if(timer_running[timer]) {
        //~ configASSERT(false);
        timer_remove_task(&TIMER, &HWTimers[timer]);
    }
    timer_running[timer] = true;
    timer_add_task(&TIMER, &HWTimers[timer]);
    //~ UART_write("RR",2);
}

void TimerStop(TimerType_t timer) {
    //~ UART_print("stop:");
    switch(timer) {
        case RxTimeoutTimer:
            //~ UART_print("RX");
...
void TimerSetValue(TimerType_t timer, uint32_t timeout ) {
    //~ UART_print("set:");
    switch(timer) {
        case RxTimeoutTimer:
            //~ UART_print("RX");
    ...
    HWTimers[timer].interval = timeout;
    //~ UART_write("RR",2);
}
...
void SX1276Init( RadioEvents_t *events )
{
	...
	HWTimers[TxTimeoutTimer].interval = 10000;
	HWTimers[TxTimeoutTimer].cb       = TxTimeoutTimer_cb;
	HWTimers[TxTimeoutTimer].mode     = TIMER_TASK_ONE_SHOT;
    
	HWTimers[RxTimeoutTimer].interval = 10000;
	HWTimers[RxTimeoutTimer].cb       = RxTimeoutTimer_cb;
	HWTimers[RxTimeoutTimer].mode     = TIMER_TASK_ONE_SHOT;
	HWTimers[RxTimeoutSyncWord].interval = 10000;
	HWTimers[RxTimeoutSyncWord].cb       = RxTimeoutSyncWord_cb;
	HWTimers[RxTimeoutSyncWord].mode     = TIMER_TASK_ONE_SHOT;
	ledblink.interval = 100;
	ledblink.cb       = led_cb;
	...
	//~ ext_irq_register(DIO0_IRQ_PIN, button_on_PA20_pressed);
	//~ ext_irq_register(DIO1_IRQ_PIN, button_on_PA21_pressed);
	//~ ext_irq_register(DIO2_IRQ_PIN, button_on_PA14_pressed);
...
void SX1276Send( uint8_t *buffer, uint8_t size )
{
    ...
    //~ UART_println("SX1276Send");

    if(SX1276.Settings.State == RF_RX_RUNNING) {    //alllow RX interrupt
    ...
    //~ UART_println("set tx2");
}

void SX1276SetSleep( void )
{
    //~ UART_println("SX1276SetSleep");
    ...
    SX1276SetOpMode( RF_OPMODE_SLEEP );
    SX1276.Settings.State = RF_IDLE;
    //~ UART_println("SX1276SetSleep, exit");
}

void SX1276SetStby( void )
{
    //~ UART_println("SX1276SetStby");
    TimerStop( RxTimeoutTimer );
    TimerStop( TxTimeoutTimer );
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        ...
        //~ UART_println("SX1276SetRx:timeout added");
    }
    ...
    {
        //~ UART_println("SX1276SetRx:opmode");
...
void SX1276SetTx( uint32_t timeout )
{
    ...
    //~ UART_println("SX1276SetTx, timer1");
    TimerStart( TxTimeoutTimer );
    SX1276SetOpMode( RF_OPMODE_TRANSMITTER );
    //~ UART_println("SX1276SetTx, return");
...
void SX1276OnTimeoutIrq(const struct timer_task *const timer_task)
{
    //~ UART_println("TimeoutIrq()");

    switch( SX1276.Settings.State )
    {
    case RF_RX_RUNNING:
        //~ UART_println("TimeoutIrq(),RX running");
        if( SX1276.Settings.Modem == MODEM_FSK )
        {
            SX1276.Settings.FskPacketHandler.PreambleDetected = false;
            SX1276.Settings.FskPacketHandler.SyncWordDetected = false;
        ...
        break;
    case RF_TX_RUNNING:
        //~ UART_println("TimeoutIrq(),TX running");
        SX1276.Settings.State = RF_IDLE;
        ...
        break;
    default:
        //~ UART_println("TimeoutIrq(), unknown");
        break;
    }
    //~ UART_println("TimeoutIrq(), exit");
}

void SX1276OnDio0Irq( void )
{
    volatile uint8_t irqFlags = 0;
    //~ DEBUG_PRINT("DIO0 irq");
    //~ UART_printInt(SX1276.Settings.State);
    //~ UART_write("\n\r", 2);
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            //~ UART_println("DIO0 irq, rx running");
            //TimerStop( &RxTimeoutTimer );
            // RxDone interrupt
            switch( SX1276.Settings.Modem )
            {
        ...
        case RF_TX_RUNNING:
            ...
            case MODEM_FSK:
            default:
                //~ UART_println("DIO0 irq, fire event");
                SX1276.Settings.State = RF_IDLE;
